...
exports.DB.prototype.autoIncreaseId = function(collName,docQuery,fieldName,callback)
{
    ...
    var inccoll = this.coll("opencomb/auto_increase_id") ;
    var db = this ;
...
exports.onregister = function(app,callback){
    ...
    {
        var sAuth = ""
        if (app.config.db.username && app.config.db.password){
            sAuth = app.config.db.username+":"+app.config.db.password+"@"
        }
        var url = "mongodb://"+ sAuth +app.config.db.server+":"+(app.config.db.port||"27017")+"/"+app.config.db.name ;
