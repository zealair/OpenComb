...
module.exports = {
	...
	, shader: function(node,buff,next,generator,tpl){
		...
		var name = text.attrToExpr(node,"name","'*'") ;
		var mode = text.attrToExpr(node,"mode","undefined") ;
		...
		buff.write("for(var loopname in $model.$view._children)") ;
		buff.write("{") ;
		buff.indent(1) ;
		buff.write("console.log('loopname:',loopname) ;") ;
		buff.write("buff.write($model.$view._children[loopname].buff,mode||'weak') ;") ;
		buff.indent(-1) ;
		buff.write("}") ;
		buff.output("</div>") ;
		buff.indent(-1) ;
