...
module.exports.FormerForControllerInstance = function (controllerIns,former)
{
	...
	this.save = function(model,beforeCallback,doneCallback)
	{
		if(!doneCallback)
		{
			doneCallback = this.controllerInstance.hold(function(err,doc){
				if(err)
				{
				...
				if( !doc )
				{
					this.nut.message("系统在保存时遇到了错误。",null,"error") ;
				}
				else
				{
					this.nut.message("内容已经保存成功。",null,"success") ;
				}
		...
		return this.__proto__.save.call(this, model, beforeCallback, doneCallback ) ;
	}

	this.fillForm = function(doc)
	{
		return this.__proto__.fillForm.call(this, this.controllerInstance.nut.model, doc ) ;
	}

	this.load = function(doc,callback)
	{
		if(!this.collection)
		{
		...
		var former = this ;
		var condition = this.docCondition(doc) ;
		if(condition)
		{
			helper.db.coll(this.collection).findOne(condition,this.controllerInstance.hold(function(err,doc){
