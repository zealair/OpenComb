...
var Controller = module.exports = Class.extend({
	...
	, main: function(req,rsp,platform)
	{
		...
		var earth = (new Earth(this,req,rsp,platform)).fillSeedByReq() ;
		var sumsigns = earth.seed["@sumsigns"]? earth.seed["@sumsigns"].split(","): [] ;

		// 执行主控制器
		processController.call( this , earth, boolean(earth.seed,"@layout",true) ) ;


		rsp.on("complete",function(){
	...
	, _loadView: function(callback){
		...
		var viewfullpath = require.resolve(this.view) ;
		var controller = this ;
...
//				if( !define.process )
//				{
//					callback(new Error("controller definement invalid: "+pathname+". controller file have to define and export a function named \"process\".")) ;
//					return ;
//				}
