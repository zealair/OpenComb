...
module.exports.prototype.load = function(callback)
{
	...
	this.controllerInstance.step(function(){
		...
		var condition = former.keys() ;
		if(condition)
		{
			helper.db.coll(former.collection).findOne(condition,this.hold(function(err,doc){
...
module.exports.prototype.remove = function(callback,condition,opts)
{
	if(!this.collection)
	{
	...
	var condition = condition || this.condition() ;
	if(!condition)
	{
	...
	var collection = helper.db.coll(former.collection) ;
	var former = this ;
	var deldoc ;
	...
	this.controllerInstance.step(
		...
		, function(err,doc){
			...
			this.each(former.widgets,function(name,widget){
				...
				{
					var oriName = former.options["func.file.restorName"] (doc[name]) ;
		...
		, function(err,affe){
			...
			if(err)
			{
				...
				former.message(opts["msg.remove.error"],"error") ;
			}
			else
			{
				if( !affe )
