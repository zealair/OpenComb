...
exports.init = function(controller,callback,sync)
{
	...
	{
        helper.log.trace("waiting controller:",controller.pathname) ;
		return ;
	}

    helper.log.trace("init controller:",controller.pathname) ;
	...
	this._loadView(controller,function(){
		...
		// formers
		if(controller.viewTpl)
		{
			controller.formers = Former.createFormersFromTemplate(controller.viewTpl) ;
		}
...
exports._loadDone = function(controller)
{
    ...
    helper.log.trace("loaded controller:",controller.pathname) ;
	...
	while( (callback=controller._initCallbacks.shift())!==undefined )
	{
        ...
        {
            helper.log.error(controller._initializeErr) ;
        }
...
{
//    app().helper.logger.trace("constroller ready:",controller.pathname) ;
//    var _deps = "" ;
//    for(var i=0;i<this._deps.length;i++)
//        _deps+= this._deps[i].pathname + ", " ;
//    app().helper.logger.debug("remain:",this._deps.length,_deps) ;
