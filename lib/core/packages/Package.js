...
module.exports.prototype.load = function(folder,callback)
{
	...
	Steps(
		...
		, function(err,files){
			...
			{
				self.addInnerPackage(this.modulesFolder+"/"+files[i],this.hold(function(err){
					if(err) throw err ;
				})) ;
			}
		}
...
module.exports.prototype.addInnerPackage = function(folder,callback)
{
	...
	pkg.load(folder,function(err){
		...
		if( !err && !self.inners[ pkg.name ] )
		{
			self.inners[pkg.name] = pkg ;
			self.inners[pkg.folderName] = pkg ; // 同名pacakge中,folder name 的优先级高于 package.json中的name
			pkg.parent = self ;
		}
...
function firstSlide (pt,start){
	start || (start=0) ;
	if(!pt) return [""] ;

	var pos = pt.indexOf('/',start) ;
	if(pos<0)
	{
	...
	{
		return [pt.slice(start,pos),pt.substr(pos+1)] ;
	}
}
...
module.exports.prototype.locate = function(pt)
{
	...
	if(pt[0]=='/')
	{
		if( pt==this.folder || pt==this.folder+"/" )
		{
			return this ;
		}

		if( pt.substr(0,this.folder.length+1)==this.folder+'/' )
		{
...
module.exports.prototype.innerPackage = function(innerpath)
{
	...
	if(innerpath.substr(0,13)=="node_modules/")
	{
		var arr = firstSlide(innerpath,13) ;
		var depName = arr[0] ;
		var subpt = arr[1] ;
