...
module.exports.prototype._createBaseMiddlewares = function(application){
    ...
    // bower support !
    application.connect.use(public(this.rootdir+'/bower_components','/')) ;

    // functional middlewares
    application._connetBaseMiddlewares = {
        "oc.shipper": application.shipper.middleware.bind(application.shipper)
	, "connect.favicon": connect.favicon()
	, "connect.cookie": connect.cookieParser()
	, "connect.query": connect.query()
	, "connect.session": connect.session({ secret: 'opencomb application', cookie: { maxAge: 7200000 }, store: application.sessionStore })
	, "connect.body": connect.bodyParser()
	, "oc.doc": DocViewer.middleware
	, "oc.router": application.router.route.bind(application.router)
	, "oc.notfound": function(req,rspn){
	    rspn.statusCode = "404" ;
