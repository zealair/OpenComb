...
module.exports.prototype.connect_middleware = function(req,rspn,next){
	...
	if( urlInfo.pathname.substr(0,11)=="/ship-down:" )
	{
		...
		query.callwrapper = query.callwrapper || "$define" ;
		...
		this.load( path, function(err,compiled){
			...
			if(err)
			{
				if( query.errcallwrapper )
				{
					var errcallwrapper = query.errcallwrapper.replace("$path",'"'+path+'"') ;
					errcallwrapper = errcallwrapper.replace("$message",'"'+err.message+'"') ;
					rspn.write(errcallwrapper) ;
				}
				else
				{
					rspn.statusCode = "403" ;
			...
			{
				var callwrapper = query.callwrapper.replace("$deps",JSON.stringify(compiled.deps||[])) ;
				callwrapper = callwrapper.replace("$path",'"'+path+'"') ;
				callwrapper = callwrapper.replace("$define",compiled.code) ;
				rspn.write(callwrapper) ;
			}
...
module.exports.prototype.load = function(path,callback){
	...
	fs.readFile(fullpath,function(err,buff){
		...
		while( res=regexp.exec(source) )
		{
			...
			if(!res2)
			{
				callback({code:500,message:"cound't pass a variable \""+res[2]+"\" to require(). file: "+path})
				return ;
			}
