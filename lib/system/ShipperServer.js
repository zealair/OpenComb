...
module.exports.prototype.connect_middleware = function(req,rspn,next){
	...
	// ------------------------------------------------------------
	// 下载前端框架
	if(urlInfo.pathname=='/opencomb/frontend')
	{
		rspn.statusCode = "200" ;
	...
	var res = /^\/shipdown(:([^\/]+))?(\/(.*))?$/.exec(urlInfo.pathname) ;
	if(!res)
	{
		next() ;
		return ;
			...
			else
			{
				if( !controller.viewScriptCache )
				{
					...
					if( controller.frontend )
					{
						...
						{
							if( controller.frontend[props[i]] )
							{
								...
								source+= "	}\r\n" ;
								source+= "	var $ = this.$ ;\r\n" ;
								source+= "	("+controller.frontend[props[i]].toString() + "\r\n";
	...
	else if( type=='tpl' )
	{
		...
		ViewTemplateCaches.singleton.template(path,function(err,tpl){
			if(err)
			{
				rspn.statusCode = "404" ;
...
module.exports.prototype.buildFrontendFramework = function(callback)
{
	...
	var __ocFrameworkFrontendRequires = [
	...
	Step(
		...
		, function parseRequireDeps(err,buff){
			...
			if( !Platform.singleton.config.dev.frontend.dontBundle )
			{
				var modules = [] ;
				var waitingloads = [] ;
...
Caches.prototype.load = function(path,callback)
{
	...
	if( !this._moduleCaches[path].loading )
	{
		...
		fs.readFile(path,function(err,buff){
			...
			//
			if( caches.platformConfig.dev.watching.shipdown )
			{
				fs.unwatchFile(path) ;
