...
var Controller = module.exports = Class.extend({
	...
	, _loadView: function(callback){
		...
		View.Caches.singleton.load(this.view,null,function(err,tpl){
			controller.view = tpl ;
			if(err)
			{
			...
			var onshowScript = viewfullpath.substring(0,viewfullpath.length-path.extname(viewfullpath).length) + ".js" ;
			fs.exists(onshowScript,function(exist){
				if(exist)
				{
					controller.view.onshowScript = path.relative(process.cwd()+"/node_modules",onshowScript) ;
				}
...
Controller.Output.prototype.assembleView = function(resourcepool)
{
	// 共享 resourcepool
