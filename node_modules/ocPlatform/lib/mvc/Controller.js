...
var Controller = module.exports = Class.extend({
	...
	, main: function(req,rsp,platform)
	{
		var processController = function(params,layout)
		{
			...
			var nut = new Nut(this,rsp) ;

			// 计算摘要签名
			this.evalSummarySignature(params,nut) ;

			// 检查客户端缓存签名
			if( utilarray.search(sumsigns,nut.model.$sumsign)===false )
			{
				// 执行自己
				nut.hold() ;
				if( this.process(params,nut,platform)===true )
				{
					nut.release() ;
				}
			}
			...
			{
				nut._children.layout = arguments.callee.call(this.layout,params.$layout,true) ;
				nut._children.layout.view.wrapperClasses.push("oclayout") ;
			}

			// 执行子控制器
			for(var name in this.children)
			{
				nut._children[name] = arguments.callee.call(this.children[name],params["$"+name],false) ;
			}
		...
		var nut = processController.call(
				this
				, params
		...
		rsp.on("complete",function(){
			...
			{
				var json = JSON.stringify( nut.cleanup() ) ;
